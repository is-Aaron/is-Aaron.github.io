...
| 候选者类型                   | 描述                                   | 优先级 | 使用场景            |
| :--------------------------- | :------------------------------------- | :----- | :------------------ |
| **Host**                     | 设备的本地 IP 地址                     | 最高   | 同一局域网内的设备  |
| **Server Reflexive (srflx)** | 通过 STUN 服务器获取的公网 IP 和端口   | 高     | 穿透简单 NAT        |
| **Peer Reflexive (prflx)**   | 在 ICE 连接性检查过程中发现的对等方地址 | 中     | 对等连接过程中发现  |
| **Relay**                    | 通过 TURN 服务器中继的地址             | 最低   | 严格防火墙/对称 NAT |
...
**安全特性**：
- 所有媒体流和数据通道都加密，无法被中间人窃听
- 使用自签名证书，通过 SDP 的 `fingerprint` 字段验证
- 即使在不安全的网络（如公共 WiFi）中也能保证通信安全
...
| **强制加密** | 端到端加密（DTLS/SRTP），保护通信隐私和安全                  |
