...
2. **输出生成方式**：模型逐个生成输出 token，每次预测下一个最可能的 token。每个新生成的 token 只能依赖于之前已生成的内容，不能"看到"未来的内容
...
- **推理成本**：生成长文本需要多次前向传播，计算成本随输出长度线性增长
...
- **RAG（检索增强生成）**：接入实时数据源和知识库，解决知识过时问题
- **向量数据库**：存储和检索长期对话历史，突破上下文窗口限制
...
- **多模态训练**：训练能处理视觉、语音等多模态输入的模型，扩展应用场景
- **RLHF（人类反馈强化学习）**：在预训练后，通过人类偏好数据训练奖励模型，再用强化学习算法优化模型的对齐性和输出质量
