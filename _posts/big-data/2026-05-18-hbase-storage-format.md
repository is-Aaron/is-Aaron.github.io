...
key_bytes
value_bytes
tags_length    # HFile v3 且 FileInfo 中存在 MAX_TAGS_LEN 时，2 bytes
tags_bytes
```
...
timestamp DESC
type（同一 row/family/qualifier/timestamp 下再比较）
```
...
sequenceDiagram
    ...
    RS->>WAL: sync before success
    WAL->>FS: filesystem sync
    Note over WAL,FS: SYNC_WAL flushes to filesystem, FSYNC_WAL forces disk
    RS-->>C: write success
    MS-->>FS: later flush to StoreFile / HFile
```
...
3. 默认 `SYNC_WAL` 下，本次修改在返回成功前会等待 WAL 同步到文件系统实现；以 HDFS 为例，这意味着 flush 到指定数量的 DataNode，但不等同于 `FSYNC_WAL` 的强制落盘。实际实现可能对多个写入做 WAL sync batching，但客户端成功返回仍要满足该 mutation 所需的持久性语义。如果 WAL 写入或同步失败，本次修改不会对客户端报告成功。
...
flowchart TD
    ...
    HF --> BF["Bloom Blocks<br/>跳过确定不命中的文件"]
...
Bloom Filter 用来减少无效 HFile 读取。因为一个 Store 里可能有多个 HFile，而某个 RowKey 或 Row+Column 未必存在于每个 HFile 中。Bloom Filter 可以快速判断“这个文件确定不包含目标 key”，从而跳过不必要的磁盘访问。
...
| `ROWCOL` | RowKey + ColumnFamily + Qualifier 级别 | 同一行列很多，点查具体列 |
...
HFile v3 在 v2 结构基础上增加了对 cell tags 的支持，也为透明加密、cell-level ACL、可见性标签等安全能力提供必要的格式支持；如果 HFile 启用了加密，v3 Trailer 中会保存该文件的数据密钥信息。当前 HBase 默认配置中，`hfile.format.version` 是 `3`。如果读旧版本资料时看到 HFile v1/v2，需要注意它们是格式演进历史，不应直接当作现代集群的新文件默认格式。
...
flowchart TD
    ...
    SF --> BF{"Bloom Filter<br/>一定不包含目标 key?"}
    BF -->|yes| SKIP["Skip HFile"]
    BF -->|no / disabled| BI["Block Index<br/>定位 Data Block"]
    BI --> BC{"BlockCache 命中?"}
...
| Major Compaction | 对某个 Store 做完整重写 | 常规 Store 默认产出每个 Store 一个 StoreFile，并处理 tombstone、TTL、最大版本等；Stripe/MOB 或特殊 compaction policy 可能有额外行为 |
...
**压缩** 面向 HFile block 的整体字节流，目标是减少磁盘占用和 I/O；对大 value 尤其明显。常见选择包括 Snappy、LZ4、GZ、ZSTD 等，具体可用性取决于 HBase/Hadoop 版本和集群安装的 native codec。
...
写入伪代码如下，其中 `delete_column` 表示内部 `DeleteColumn` tombstone，用来遮蔽该列所有时间戳小于等于 t5 的版本；它不是在描述 Java API 中 `Delete.addColumn` / `Delete.addColumns` 的方法名差异。
...
等后续 major compaction 发生后，`info:name` 的旧值和 tombstone 才可能在满足 `KEEP_DELETED_CELLS`、delete TTL、未来时间戳等配置约束时被真正清理掉。
...
| Compaction | 合并多个 StoreFile 的后台过程，用于减少文件数，并在满足条件时清理过期版本和删除标记。 |
| Tombstone | 删除标记。Delete 操作写入 tombstone，后续读取时遮蔽旧数据，并在满足条件的 compaction 中清理。 |
...
9. Apache HBase Javadocs, [CellComparator](https://hbase.apache.org/devapidocs/org/apache/hadoop/hbase/CellComparator.html)
10. Fay Chang et al., [Bigtable: A Distributed Storage System for Structured Data](https://research.google/pubs/bigtable-a-distributed-storage-system-for-structured-data/)
